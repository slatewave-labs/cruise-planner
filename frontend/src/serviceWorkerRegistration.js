...
// Based on the CRA PWA template with additions for update notification and cache busting.
...
export function register(config) {
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', () => {
      ...
      if (isLocalhost) {
        // In development, check if the SW still exists and is valid
        ...
        installingWorker.onstatechange = () => {
          if (installingWorker.state === 'installed') {
            if (navigator.serviceWorker.controller) {
              // New content is available — trigger cache busting
              console.log('[SW] New content available; will update on next reload.');
              if (config && config.onUpdate) {
                config.onUpdate(registration);
              }
            } else {
              // First install — all content precached for offline use
