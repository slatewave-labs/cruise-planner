...
- **Backend (Python)**: pytest, pytest-asyncio, httpx (for async FastAPI testing), `unittest.mock` for DynamoDB/external service mocking
...
4. **Mock external services, not your own code.** Mock the Groq API (via `LLMClient`), Open-Meteo API, and any AWS calls — not your own utility functions.
5. **Edge cases matter.** Empty inputs, missing fields, network timeouts, invalid coordinates, special characters in ship names.
6. **Test the sad paths.** What happens when the AI returns garbage JSON? When localStorage is full or corrupted? When the weather API returns a 500?
...
- Utility functions (`utils.js`, `storage.js`, data transformers)
- Component rendering (does it show the right content for given props?)
- Weather code mapping
- Port data lookups
- localStorage read/write helpers (handle full storage, invalid JSON)

### Integration Tests
- FastAPI endpoint testing with httpx AsyncClient
- Full request/response cycle for AI plan generation and port search
