...
if [[ -n "$CERTIFICATE_ARN" ]]; then
    ...
    if [[ -n "$EXISTING_CERT" && "$EXISTING_CERT" != "None" ]]; then
        ...
        echo "    3. Click 'Create records in Route 53' (if using Route 53)"
        echo "       OR add the CNAME records shown to your DNS provider"
        echo "    4. Wait 5-30 minutes for validation to complete"
...
if [[ "$DOMAIN_NAME" != "$ALB_DNS" ]]; then
    ...
    echo "    1. Go to your DNS provider (Route 53, GoDaddy, Namecheap, etc.)"
    echo "    2. Create a CNAME record:"
    echo "       Name:  $DOMAIN_NAME (or @ for root domain)"
    echo "       Type:  CNAME (or ALIAS if using Route 53 for root domain)"
    echo "       Value: $ALB_DNS"
    echo "       TTL:   300 (or default)"
    echo ""
    echo "  For Route 53 (recommended):"
