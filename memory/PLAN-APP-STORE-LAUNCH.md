...
- [x] Update `manifest.json` with all required fields
- [x] Create maskable icons (192×192 and 512×512)
- [x] Implement service worker for offline (with dedicated offline fallback page)
- [x] Set up Bubblewrap project (`twa-manifest.json`)
- [x] Create Digital Asset Links file (`.well-known/assetlinks.json`)
- [x] Create CI/CD pipeline for beta APK builds (`build-android-beta.yml`)
- [x] Create CI/CD pipeline for production AAB builds (`build-android-prod.yml`)
- [x] Fix colour contrast issues (accessibility audit)
- [x] Create step-by-step publishing guide (`docs/GOOGLE-PLAY-PUBLISHING-GUIDE.md`)
- [ ] Build and sign AAB bundle *(manual — run workflow after configuring secrets)*
- [ ] Create Google Play Developer account *(manual — $25, see publishing guide)*
- [ ] Prepare store listing — screenshots, description, feature graphic *(manual)*
- [ ] Upload and submit for review *(manual)*
...
> Audit run locally against production build (March 2026).
> Note: Performance score is lower than production due to local testing conditions (no CDN, HTTP/2, or server-side caching).
...
- ✅ Mobile nav labels increased from 10px to 11px
- ✅ Separate maskable icon entries added to manifest
- ✅ Offline fallback page created and integrated with service worker
- ⚠️ Performance: render-blocking Google Fonts & Leaflet CSS (expected, CDN-cached in production)
...
|---|---|---|
| `frontend/public/manifest.json` | MODIFIED | Separate maskable icons, added `id`, `lang`, `dir`, `prefer_related_applications` |
| `frontend/public/.well-known/assetlinks.json` | CREATED | Digital Asset Links for TWA verification |
| `frontend/public/offline.html` | CREATED | Offline fallback page |
| `frontend/public/icon-maskable-192.png` | CREATED | Maskable icon 192×192 |
| `frontend/public/icon-maskable-512.png` | CREATED | Maskable icon 512×512 |
| `frontend/src/service-worker.js` | MODIFIED | Added offline fallback for navigation failures |
| `frontend/src/components/Layout.js` | MODIFIED | Fixed nav text size and colour contrast |
| `frontend/src/components/CookieBanner.js` | MODIFIED | Fixed link colour contrast |
| `frontend/tailwind.config.js` | MODIFIED | Added `accent.dark` colour variant |
| `twa-manifest.json` | CREATED | Bubblewrap TWA configuration |
| `.github/workflows/build-android-beta.yml` | CREATED | Beta APK build pipeline |
| `.github/workflows/build-android-prod.yml` | CREATED | Production AAB build pipeline |
| `docs/GOOGLE-PLAY-PUBLISHING-GUIDE.md` | CREATED | Step-by-step manual publishing guide |
